...
'$location', 'MakeAWishFactsService', '$dialog', '$http', '$q',
($scope, game, $timeout,
$location, MakeAWishFactsService, $dialog, $http, $q) => {
  ...
  $scope.getQuestions = region => (
    $http.post(`/questions/${region}`)
  );

  $scope.getAnswers = region => (
    $http.post(`/answers/${region}`)
  );
  ...
  $scope.shuffleCards = () => {
    ...
    setTimeout(() => {
      $q
      .all([$scope.getAnswers($scope.region),
        $scope.getQuestions($scope.region)])
      .then(() => {
